/**
 * One-time script: set mediaType to 'movie' for all existing UploadedVideo documents
 * that don't have mediaType or have it null (backfill after adding mediaType to schema).
 * Run from backend: node scripts/set-uploaded-video-media-type-movie.js
 *
 * If you see 0 documents but the app shows uploaded videos, the script is likely using a
 * different DB than the running server. Fix: pass the SAME MONGODB_URI the server uses:
 *   Windows (PowerShell): $env:MONGODB_URI="mongodb://..."; node scripts/set-uploaded-video-media-type-movie.js
...
async function run() {
  console.log('\nStarting script: set UploadedVideo.mediaType to "movie" where missing');
  ...
  console.log('Actual database name:', dbNameActual, '| Collection:', UploadedVideoModel.collection.name);
  ...
  try {
    const totalBefore = await UploadedVideoModel.countDocuments({});
    console.log('Total UploadedVideo documents:', totalBefore);

    const withoutMediaType = await UploadedVideoModel.countDocuments({
      $or: [{ mediaType: { $exists: false } }, { mediaType: null }],
    });
    const withMovie = await UploadedVideoModel.countDocuments({ mediaType: 'movie' });
    const withTv = await UploadedVideoModel.countDocuments({ mediaType: 'tv' });
    ...
    const result = await UploadedVideoModel.updateMany(filter, { $set: { mediaType: 'movie' } });
    console.log('Result: matchedCount =', result.matchedCount, ', modifiedCount =', result.modifiedCount);

    const totalAfter = await UploadedVideoModel.countDocuments({});
    const withMovieAfter = await UploadedVideoModel.countDocuments({ mediaType: 'movie' });
